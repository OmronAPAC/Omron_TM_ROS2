...
bool tmr_msgs__msg__feedback_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("tmr_msgs.msg._feedback_state.FeedbackState", full_classname_dest, 42) == 0);
  }
  tmr_msgs__msg__FeedbackState * ros_message = _ros_message;
  {  // header
  ...
  {  // flange_pose
    ...
    PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'flange_pose'");
    if (!seq_field) {
      Py_DECREF(field);
      return false;
    }
    ...
    double * dest = ros_message->flange_pose.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      PyObject * item = PySequence_Fast_GET_ITEM(seq_field, i);
      if (!item) {
        Py_DECREF(seq_field);
...
PyObject * tmr_msgs__msg__feedback_state__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("tmr_msgs.msg._feedback_state");
  ...
  {  // flange_pose
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "flange_pose");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->flange_pose.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->flange_pose.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->flange_pose.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
