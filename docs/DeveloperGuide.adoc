...
This implementation was designed using TM Landmark attached to the object to be picked, and another TM landmark to find the place location. 
...
. Run the program to teach the setup, replacing `ip_address` with the ip address of the TM robot.
+
....
ros2 run pickplace teach_setup <ip_address>
....
+
. Enter the name of the vision job done in the TMflow program by the vison node.
+
....
<vision_job_name>
....
...
image::viewpick.gif[]
+
. Hit kbd:[ENTER] and wait for the vision job to complete.
+
...
image::viewplace.gif[]
+
. Hit the kbd:[ENTER] button and wait for the vision job to complete.
+
. Move the TM arm to the pick position and hit kbd:[ENTER], which releases the grippers and records the position.
+
image::place.gif[]
+
...
. . Run the launch file to execute the program, replacing `ip_address` with the ip address of the TM robot.
+
....
ros2 launch pickplace pickplace.launch.py robot_ip:=<ip_address>
....
+
